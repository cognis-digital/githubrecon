...
- **rules/analyzers** apply the heuristics shipped in `githubrecon/core.py`.
- **score** ranks by severity.
- **MCP server** (`githubrecon mcp`) exposes `scan` for Cognis.Studio agents.
